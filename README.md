...
- **Diagram families** - whatever the real tool draws: sequence, use case, class, activity, component, state, object, deployment, timing, gantt, mindmap, WBS, JSON, YAML and the rest. Tinta does no PlantUML parsing of its own, so coverage is PlantUML's own.
